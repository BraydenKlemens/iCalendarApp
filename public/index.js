...
$(document).ready(function () {
	...
	function getLogPanelInformation() {
		$.ajax({
			...
			success: function (data) {
				let addToLog =
					"<tr><th>File Name<br/>(click to download)</th><th>Version</th><th>Product ID</th><th>Number of events</th><th>Number of properties</th></tr>";
	...
	$("#uploadButton").on("change", function (get) {
		...
		if (fn.split(".").pop() === "ics") {
			...
			let cur = $status.val();
			$status.val(
				cur + "UPLOAD ERROR:" + fn + " is not a valid calendar file\n"
			);
		}
	});
	...
	$("#view_dropdown").on("change", function () {
		...
		$.ajax({
			...
			success: function (data) {
				let add =
					"<tr><th>Event</th><th>Start Date</th><th>Start Time</th><th>Summary</th><th>Props</th><th>Alarms</th></tr>";
				let time = "";
				let date = "";
				let eventNum = 1;
				...
				if (data.events !== undefined) {
					for (let i in data.events) {
						...
						add +=
							...
							time +
							"</td><td>" +
							data.events[i].summary +
							"</td><td>" +
							data.events[i].numProps +
							"</td><td>" +
							data.events[i].numAlarms +
							"</td></tr>";
						eventNum++;
					}
				}
	...
	$("#show_alarm_btn").click(function () {
		let object = {
			name: $("#view_dropdown").val(),
			num: $("#showalarm_entry").val(),
		};
		$("#showalarm_entry").val("");
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.alarms === "Invalid File") {
					let cur = $status.val();
					$status.val(
						cur + "SHOW ALARMS ERROR:" + data.file + " is an invalid file\n"
					);
				} else if (data.alarms === "Invalid Input") {
					let cur = $status.val();
					$status.val(
						cur +
							"SHOW ALARMS ERROR: Event No. " +
							data.num +
							" does not exist\n"
					);
				} else if (data.alarms === "[]") {
					...
					$status.val(
						cur +
							"SHOW ALARMS ERROR: No alarms exist in " +
							data.file +
							" Event No. " +
							data.num +
							"\n"
					...
					for (let i in data.alarms) {
						string +=
							...
							index +
							": Action:" +
							data.alarms[i].action +
							", Trigger:" +
							data.alarms[i].trigger +
							", numProps:" +
							data.alarms[i].numProps +
							"\n";
						index++;
					}
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.props === "Invalid File") {
					let cur = $status.val();
					$status.val(
						cur + "EXTRACT PROPS ERROR:" + data.file + " is an invalid file\n"
					);
				} else if (data.props === "Invalid Input") {
					let cur = $status.val();
					$status.val(
						cur +
							"EXTRACT PROPS ERROR: Event No. " +
							data.num +
							" does not exist\n"
					);
				} else if (data.props === "[]") {
					...
					$status.val(
						cur +
							"EXTRACT PROPS ERROR: No optional properties exist in " +
							data.file +
							" Event No. " +
							data.num +
							"\n"
		...
		$.ajax({
			...
			success: function (data) {
				if (data.valid === "true") {
					...
					$status.val(
						cur +
							"CREATE EVENT ERROR: failed to add an event to " +
							object.file +
							"\n"
					);
				}
		...
		$.ajax({
			...
			success: function (data) {
				if (data.valid === "true") {
					...
					$status.val(
						cur +
							"CREATE CALENDAR ERROR: failed to create " +
							object.file +
							"\n"
					);
				}
						...
						if (data.events !== "fail") {
							...
							$status.val(
								cur +
									"FAIL:" +
									object.file +
									" doesnt exist in data base or file has no events\n"
							);
						}
					},
					error: function (error) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.alarms !== "empty") {
							...
							let cur = $status.val();
							$status.val(
								cur + "No alarms in calendar file " + object.file + "\n"
							);
						}
					},
					error: function (error) {
			...
			if ($("#choosefile_dropdown").val() !== "none") {
				$.ajax({
					...
					success: function (data) {
						...
						let cur = $status.val();
						$status.val(
							cur + "SUCCESS: deleted " + object.file + "from the data base\n"
						);
					},
					error: function (error) {
						console.log(error);
			...
			$.ajax({
				...
				success: function (data) {
					if (data.state === "success") {
						...
						let cur = $status.val();
						$status.val(
							cur + "SUCCESS: deleted upcoming event from " + object.file + "\n"
						);
					} else {
						let $status = $("#status");
						let cur = $status.val();
						$status.val(
							cur + "FAIL: no events to delete in " + object.file + "\n"
						);
					}
				},
				error: function (error) {
